...
public class ProviderService {
    ...
    private void applyRequest(Provider provider, ProviderRequest request, boolean creating) {
        ...
        if (request.images() != null) {
            provider.setImages(request.images().stream().limit(6).collect(Collectors.toCollection(java.util.ArrayList::new)));
        } else {
            provider.setImages(new java.util.ArrayList<>());
        }
